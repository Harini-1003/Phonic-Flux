...
# Initialize Gemini (using OpenAI class from LangChain)
...
def chatbot():
    ...
    if user_input == "1":
        ...
        response = "Please describe your issue or question for customer support."
        session['context'] = 'customer_support'
    elif session.get('context') == 'customer_support':
        customer_id = 'N/A'  # This can be fetched from session or user input if available
        response = customer_support_log(customer_id, user_input)
        response += "\nDo you need any more assistance? (Yes/No)"
        session['context'] = 'check_more_assistance'
    elif session.get('context') == 'check_more_assistance':
        if user_input.lower() == 'yes':
            response = display_menu()
            session['context'] = None
        else:
            response = "Thank you for using RetailX Assistant. Have a great day!"
            session['context'] = None

    elif user_input == "6":
        response = "Inventory Management is not available in this demo. Please contact support for further assistance."
        session['context'] = None

    else:
        response = "Sorry, I didn't understand that. Please choose a valid option from the menu."
